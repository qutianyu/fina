  ...
  },
  {
    ...
    action: async (args, _config, _skillManager, shellState) => {
      if (!shellState?.sessionManager) {
        console.log(picocolors.red('Session manager not initialized'));
        return;
      }
      const session = await shellState.sessionManager.createSession(args.name);
      shellState.currentSessionId = session.id;
      console.log(picocolors.green(`Created session: ${session.id} (${session.name})`));
    }
  },
  {
    name: '/session-list',
    ...
    action: async (_args, _config, _skillManager, shellState) => {
      ...
      console.log(picocolors.gray('Sessions:'));
      for (const s of sessions) {
        const marker = s.id === shellState.currentSessionId ? ' *' : '';
        console.log(`  ${picocolors.yellow(s.id)}${marker} - ${s.name} (${s.messages.length} messages)`);
      }
    }
  },
  {
    ...
    action: async (args, _config, _skillManager, shellState) => {
      ...
      if (!session) {
        console.log(picocolors.red(`Session not found: ${args.id}`));
        return;
      }
      shellState.currentSessionId = session.id;
      console.log(picocolors.green(`Switched to session: ${session.id} (${session.name})`));
    }
  }
];
