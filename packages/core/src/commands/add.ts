...
export class AddCommand {
  ...
  constructor(config: ConfigManager, skillManager: SkillManager | null = null, output?: OutputWriter) {
    this.config = config;
  ...
  async execute(source: string, isDirectory: boolean = false): Promise<void> {
    this.output.info(`Adding: ${source}`);
  ...
  async addFromUrl(url: string): Promise<void> {
    try {
      this.output.log('Fetching webpage...');
      ...
      if (skill) {
        this.output.log(`  Using skill: ${skill.name}`);
      }

      const { title, content, author } = Extractor.extract(response.data, skill ?? undefined);

      if (!content) {
        this.output.error('Failed to extract content from page');
        return;
      }

      // Clean content using LLM
      this.output.log('Cleaning content with AI...');
...
`;
      ...
      this.output.log(`✓ Saved to: ${path.relative(process.cwd(), filePath)}`);
      this.output.log(`  Title: ${title}`);

    } catch (err) {
      this.output.error(`Failed to fetch URL: ${(err as Error).message}`);
    }
  }
...
Output ONLY the cleaned content, nothing else. Do not add explanations or notes.`;
  ...
  async addFromFile(filePath: string, useTimestampDir: boolean = true): Promise<void> {
    try {
      ...
      if (!stats.isFile()) {
        this.output.error(`Not a file: ${filePath}`);
        return;
      }
      ...
      this.output.log(`✓ Copied to: ${path.relative(process.cwd(), destPath)}`);
      this.output.log(`  Size: ${(stats.size / 1024).toFixed(1)} KB`);
      this.output.log(`  Type: ${ext || 'unknown'}`);

    } catch (err) {
      this.output.error(`Failed to copy file: ${(err as Error).message}`);
    }
  }
  ...
  async addFromDirectory(dirPath: string): Promise<void> {
    try {
      ...
      if (!await fs.pathExists(resolvedPath)) {
        this.output.error(`Directory not found: ${dirPath}`);
        return;
      }

      const stats = await fs.stat(resolvedPath);
      if (!stats.isDirectory()) {
        this.output.error(`Not a directory: ${dirPath}`);
        return;
      }
      ...
      if (files.length === 0) {
        this.output.warn(`No supported files found in: ${dirPath}`);
        return;
      }

      this.output.info(`Adding ${files.length} files from: ${dirPath}\n`);
      ...
      if (imageMap.size > 0) {
        this.output.log(`  + ${imageMap.size} images referenced in markdown`);
      }
    } catch (err) {
      this.output.error(`Failed to add directory: ${(err as Error).message}`);
    }
  }
  ...
  private async copyFileInBatch(filePath: string, timestamp: number, imageMap?: Map<string, string>): Promise<void> {
    try {
      ...
      this.output.log(`  + ${path.basename(resolvedPath)}`);
    } catch (err) {
      this.output.error(`Failed to copy ${filePath}: ${(err as Error).message}`);
    }
  }
