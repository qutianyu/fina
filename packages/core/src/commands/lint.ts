...
export class LintCommand {
  ...
  async execute(): Promise<LintIssue[]> {
    const wikiDir = this.config.getWikiDir();

    this.output.info('\n🩺 Running knowledge base health check...\n');

    if (!await fs.pathExists(wikiDir)) {
      this.output.warn('⚠ Wiki directory does not exist. Run /make first.');
      return this.issues;
    }

    this.output.progress('lint', 'Checking for orphaned pages...');
    this.issues.push(...await this.findOrphanedPages(wikiDir));

    this.output.progress('lint', 'Checking for dangling references...');
    this.issues.push(...await this.findDanglingReferences(wikiDir));

    this.output.progress('lint', 'Checking for contradictions...');
    this.issues.push(...await this.findContradictions(wikiDir));

    this.output.progress('lint', 'Checking for stale content...');
    this.issues.push(...await this.findStaleContent(wikiDir));

    this.output.progress('lint', 'Checking for missing links...');
...
If no contradictions found, respond with: NONE`;
    ...
    try {
      if (!await this.config.ensureConfigured()) {
        this.output.warn('  ⚠ LLM not configured, skipping contradiction check');
        return issues;
      }
      ...
      this.output.warn(`  ⚠ LLM contradiction check failed: ${(err as Error).message}`);
    }
  ...
  private printReport(): void {
    ...
    if (errors.length > 0) {
      ...
      if (uniqueDangling.length > 0) {
        this.output.log(`  Missing concepts: ${uniqueDangling.join(', ')}`);
      }
    }
    ...
    if (warnings.length > 0) {
      ...
      if (orphanFiles.length > 0) {
        this.output.log(`  Orphan pages: ${orphanFiles.length}`);
      }
    }
